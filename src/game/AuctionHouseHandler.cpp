...
void WorldSession::SendAuctionOutbiddedMail(AuctionEntry *auction, uint32 newPrice)
{
	...
	ObjectGuid bidder_guid = ObjectGuid(HIGHGUID_PLAYER, auctionbot.GetAHBplayerGUID());
    ...
    if(oldBidder || oldBidder_accId)
    {
        ...
        if (oldBidder && !_player)
            oldBidder->GetSession()->SendAuctionBidderNotification( auction->GetHouseId(), auction->Id, bidder_guid, newPrice, auction->GetAuctionOutBid(), auction->item_template);
